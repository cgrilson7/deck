 ...
 * order. The first empty cell is the +, unless every slot is open. The vocabulary and
 * translator tiles take the last cells (vocab left of translate) and main's cap is one lower
 * per tile shown, so the counts still agree.
 */
export function Grid({ sessions, state, settings }: { sessions: SessionView[]; state: DeckState; settings: DeckSettings }) {
  ...
  const cells = sessionCells + Number(settings.showVocab) + Number(settings.showTranslate)
